...
- **D2 -- `.agents/skills/` and the AgentQuilt adapter coincidence (section
  ...
  a style question.
  **2026-07-14 follow-up:** the same question was raised again, this time
  narrowed to just the 14 `.codex/agents/*.toml` files (the most rigidly
  templated hand-authored set, with a direct `.claude/agents/*.md`
  counterpart) rather than skills. Re-investigation found the compiler
  route still doesn't hold even at that narrower scope: 2 of 14 names
  don't map 1:1 (`repository-analyst`/`repository-explorer` are a rename;
  `test-engineer`/`test-reviewer` are intentionally different roles per
  this document's own D5), and even matched pairs' `developer_instructions`
  bodies are independently authored, not copied, from `.claude/agents/`'s
  compiled text -- a compiler can't reproduce prose it never wrote. ADR-0012
  point 2 was not reversed. The adopted mitigation instead:
  `scripts/check-pipeline-agent-drift.mjs` (wired into `test.yml` as a
  non-blocking step), a small deterministic script -- not a compiler
  adapter, touches nothing under `packages/agentquilt-cli/src/` -- that
  checks roster parity and permission-vs-sandbox_mode parity between the
  two rosters, with the known name and permission exceptions above encoded
  and commented directly in the script.
